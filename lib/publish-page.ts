...
import { readWpConfig, wpPublishPage } from "@/lib/wordpress";

const DS_VERSION = "1.0.0";

export async function executePublishPage(
  rawInput: unknown,
): Promise<ToolResponse<PublishPageData>> {
  const timestamp = new Date().toISOString();

  const parsed = PublishPageInputSchema.safeParse(rawInput);
  if (!parsed.success) {
    return {
      ok: false,
      error: {
  ...
  if (!cfg.ok) {
    logger.error("publish_page.execute.missing_wp_env_vars", { missing: cfg.missing });
    return {
      ok: false,
      error: {
        code: "INTERNAL_ERROR",
  ...
  const wp = await wpPublishPage(cfg.value, parsed.data.page_id);
  if (!wp.ok) {
    return {
      ok: false,
      error: {
