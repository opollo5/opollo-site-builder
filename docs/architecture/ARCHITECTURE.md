...
- **Page chrome** (Spec 02 + Spec 04 amendment 2026-05-08): admin and account pages compose `components/ui/page-header.tsx` (compound component) inside `components/ui/page-shell.tsx` (1280px max-width frame). The compound enforces a fixed render order regardless of JSX child position: **Title → Breadcrumb → Subtitle → Meta → Actions**, with Actions vertically centered against the title row at the top. Vertical rhythm: 20px title→breadcrumb, 8px breadcrumb→subtitle, 12px subtitle→meta, 32px header→content (`mb-8`). Page-heading type scale lives in `app/globals.css` (`.text-page-title` 28px / 24px mobile / weight 700, `.text-section-title` 20px / weight 600, `.text-subsection` 16px / weight 600). Audit rules `headings-use-page-header`, `breadcrumb-required-when-page-header`, and `no-raw-h1-in-pages` (HIGH severity, all in `scripts/audit.ts`) enforce the contract on every PR. Two allowlists in the audit script: `PAGE_HEADER_DEFERRED_ROUTES` (temporary, drains to `[]` when Spec 04 sweep completes) and `PAGE_HEADER_EXEMPT_ROUTES` (permanent, for routes that legitimately can't host PageHeader — full-bleed editors, modal pages).
