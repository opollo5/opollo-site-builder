...
- **[M15-4 #12] Malformed JSON behavior inconsistent.** Old pattern (`try { body = req.json() } catch { body = {} }`) gives confusing "missing field" error; new pattern (`lib/http.readJsonBody`) gives clear "Request body must be valid JSON." Partial fixes 2026-05-03 — PR #500 (24 `admin/*`), PR #501 (11 `auth/account/ops/emergency/approve/sites/*`), PR #502 (4 `tools/*`), PR #503 (22 `platform/social/*`). Remaining: `optimiser/*` (~3 silent-swallow routes).
