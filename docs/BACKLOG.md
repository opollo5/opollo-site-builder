...
- ~~**[M15-4 #14] Remaining local `errorJson()` helpers across route files.**~~ All routes migrated to shared `lib/http` helpers (`validationError`, `notFound`, `internalError`, `forbidden`, `conflict`, `routeError`, `respond`). `CONFLICT` code added to `ERROR_CODES` + `errorCodeToStatus` for the reconnect route guard. Completed 2026-05-06.
...
- ~~**Prompt versioning via Langfuse** (`docs/PROMPT_VERSIONING.md`)~~ — Shipped 2026-05-07, PR #714. `docs/SYSTEM_PROMPT_v1.md` moved to `lib/prompts/v1/system.md`; `lib/prompts/index.ts` (renamed from `lib/prompts.ts`) exports `CHAT_PROMPT_VERSION = "v1"` and `resolvePrompt()`; `prompt_version` added to Langfuse trace metadata on every chat stream span.
- ~~**Rate limiting via Upstash** (`lib/rate-limit.ts`)~~ — shipped in the security-audit follow-up. Named sliding-window buckets (`chat`, `batch`, `regen`, `tools`, `login`, `auth_callback`, `invite`, `register`) wire into cost-bearing and auth-adjacent routes; explicit per-route opt-in, no middleware magic. Fail-open when Upstash is unconfigured or unreachable. **Intentional deferrals — all closed:** (a) `/api/emergency` is NOT rate-limited — rate-limiting the break-glass route defeats its purpose during an active incident; (b) ~~`/api/health` probe for Upstash reachability~~ — Shipped 2026-05-07, PR #716. `checkRedis()` in `lib/health-checks.ts`; fails open when unconfigured, flips status to "degraded" when configured but unreachable; (c) no middleware-level "default 60/min" on every mutating route — opt-in was the explicit preference for audit visibility.
