...
-- ─────────────────────────────────────────────────────────────
-- helper function (idempotent — only creates if missing)
-- ─────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION auth.user_belongs_to_company(target_company_id uuid)
RETURNS boolean
LANGUAGE sql
SECURITY DEFINER
AS $$
  ...
  USING (
    EXISTS (
      ...
      WHERE d.id = social_post_analytics_cache.draft_id
        AND auth.user_belongs_to_company(d.company_id)
    )
  );
