...
## Phase 4.2 — Link preview with OG fetch (B4) (2026-05-20)

**D-036**: Link preview URL detection — regex on value, not full URL parser
- OG preview should trigger when a URL appears anywhere in the textarea value.
- Decision: `/https?:\/\/[^\s]+/i` regex on `value` in a debounced `useEffect`. First match wins. The debounce is 250ms (spec-exact). Dismissed URLs are tracked by value in `linkPreviewDismissed` state to suppress re-trigger.

**D-037**: Link preview dismissed state resets per URL not per session
- Spec says "dismiss suppresses preview for that URL in the session". Implemented as a single `linkPreviewDismissed: string | null` — dismissing replaces the suppressed URL with the current one. If the user later types a different URL, that URL will preview normally.

---

## Phase 4.3 — UTM builder rebuild (B5) (2026-05-20)

**D-038**: UtmBuilderPanel extracted from ToolsRow inline function
- Inline `UtmPanel` in ToolsRow was basic (4 fields, no preview). The rebuild adds structured form, live preview, auto-detect, advanced section, localStorage persistence.
- Decision: Extract to `components/social/composer/UtmBuilderPanel.tsx` following the EmojiPickerPanel pattern. ToolsRow imports it; code-split by Next.js.

**D-039**: Auto-detect source defaults off when no platforms provided
- `autoSource=true` by default but `source = autoSource ? autoSourceValue : manualSource`. When `platforms=[]`, `autoSourceValue=""` which means the source param is omitted from the URL.
- Decision: `source = (autoSource && platforms.length > 0) ? autoSourceValue : manualSource` — when no platforms, manual source is used regardless of toggle. This is the correct UX: the toggle only matters when platform context is available.

**D-040**: `platforms` prop threaded ContentEditor → ToolsRow → UtmBuilderPanel
- Platforms needed for auto-detect but ToolsRow was props-isolated. No DB/API change needed — `platforms` is already computed in `ComposerEditor` from the selected connections.
- Decision: Add `platforms?: Platform[]` to `ContentEditorProps` and `ToolsRowProps`. ComposerEditor already has the `platforms` array; it now passes it to `ContentEditor`.
