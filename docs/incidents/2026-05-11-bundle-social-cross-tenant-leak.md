# Incident — bundle.social cross-tenant LinkedIn leak (investigation)

**Status:** Investigation complete. **No fix applied — separate PR will come.**
**Reporter:** Steven Morey
**Investigator:** Claude (autonomous)
**Date:** 2026-05-11
**Branch:** `investigate/bundle-social-cross-tenant-leak`
**Severity:** Medium — exploitable by any operator/admin who connects the *same* LinkedIn person account across more than one company in the same browser session. Currently scoped to test/dogfood data; one human (Steven) is the OAuth grantor for 4 different bundle.social teams.
