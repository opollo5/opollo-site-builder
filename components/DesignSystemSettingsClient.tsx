...
// ─── Default values (mirrors app/globals.css and lib/design-system/tokens.ts) ─
...
export function DesignSystemSettingsClient({ initialSettings }: Props) {
  ...
  // Current form values. null = use default.
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-xl font-display font-semibold">Design system settings</h1>
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_400px]">
        ...
        <div className="space-y-6">
          {/* Colours */}
          <section>
            <h2 className="mb-3 text-base font-semibold font-display">Colours</h2>
          ...
          {/* Typography */}
          <section>
            <h2 className="mb-3 text-base font-semibold font-display">Typography</h2>
            <div className="grid gap-3 sm:grid-cols-2">
              {FIELDS.filter((f) => f.type === "length" || f.type === "text").filter(
                (f) => f.key.startsWith("font"),
              ).map((f) => (
                <TextField
                  key={f.key}
                  spec={f}
          ...
          {/* Geometry */}
          <section>
            <h2 className="mb-3 text-base font-semibold font-display">Geometry</h2>
            <div className="grid gap-3 sm:grid-cols-2">
        ...
        {/* Right: live preview */}
        <div className="lg:sticky lg:top-8 self-start space-y-2">
          <h2 className="text-base font-semibold font-display">Live preview</h2>
    ...
    <div className="flex items-center gap-3 rounded-lg border border-border bg-card p-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <input
          ...
          value={value}
          onChange={(e) => onChange(e.target.value)}
          className="mt-1.5 w-full rounded border border-border bg-background px-2 py-1 text-xs font-mono"
...
function TextField({ spec, value, isOverridden, onChange }: TextFieldProps) {
  return (
    <div className={cn("rounded-lg border border-border bg-card p-3 space-y-1.5")}>
      <div className="flex items-center gap-1.5">
...
// ─── Preview HTML ─────────────────────────────────────────────────────────────
//
// Minimal preview document loaded in the sandboxed iframe. Uses the same
// CSS variables as the main app, so changes inject cleanly via the style tag.
...
.lbl::before { content: '—'; color: var(--gr); font-weight: 700; }
.pk { color: var(--pk); }
