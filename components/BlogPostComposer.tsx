...
import {
  AiPrefillModal,
  type AiPrefillApplyPayload,
} from "@/components/AiPrefillModal";
...
// Convert basic markdown to HTML for populating the Tiptap editor from AI output.
// Handles headings, bold, unordered/ordered lists, and paragraphs. No external dep.
function markdownToHtml(md: string): string {
  ...
  for (const raw of lines) {
    ...
    if (ul) {
      ...
      out.push(`<li>${inlineFormat(ul[1]!.trim())}</li>`);
      continue;
    }
    ...
    if (ol) {
      ...
      out.push(`<li>${inlineFormat(ol[1]!.trim())}</li>`);
      continue;
    }
    closeLists();
...
export function BlogPostComposer({ siteId }: { siteId: string }) {
  ...
  const applyAiPrefill = useCallback(
    (payload: AiPrefillApplyPayload) => {
      if (!title.touched && title.value === "" && payload.title) {
        setTitle({ value: payload.title, source: "ai", touched: true });
      }
      if (!metaTitle.touched && metaTitle.value === "" && payload.seo_title) {
        setMetaTitle({ value: payload.seo_title, source: "ai", touched: true });
      }
      if (!metaDescription.touched && metaDescription.value === "" && payload.meta_description) {
        setMetaDescription({ value: payload.meta_description, source: "ai", touched: true });
      }
      if (!slug.touched && slug.value === "" && payload.slug) {
        setSlug({ value: payload.slug, source: "ai", touched: true });
      }
      if (isEditorEmpty(composerValue.text) && payload.content) {
        setComposerValue((prev) => ({
          ...prev,
          text: markdownToHtml(payload.content),
        }));
      }
      if (selectedCategories.length === 0 && payload.categories.length > 0) {
        setSelectedCategories(payload.categories as WpTaxonomyOption[]);
      }
      if (selectedTags.length === 0 && payload.tags.length > 0) {
        setSelectedTags(payload.tags as WpTaxonomyOption[]);
      }
    },
    [
      title, metaTitle, metaDescription, slug, composerValue.text,
