...
export function BlogPostComposer({ siteId }: { siteId: string }) {
  ...
  async function handleSaveDraft(e: FormEvent<HTMLFormElement>) {
    ...
    setSubmitting(true);
    try {
      // Fix 2 — collision retry: attempt the base slug, then slug-2, slug-3 … slug-5.
      const baseSlug = slug.value.trim();
      let attempt = 0;
      ...
      while (attempt < MAX_ATTEMPTS) {
        ...
        if (payload?.ok) {
          // Update the slug field to reflect the accepted slug.
        ...
        // Retry silently on slug collision.
        if (code === "UNIQUE_VIOLATION" && attempt < MAX_ATTEMPTS - 1) {
          attempt++;
          continue;
        }
